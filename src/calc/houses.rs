...
impl RelativeEq for HousePosition {
    ...
    fn relative_eq(
        ...
        self.number == other.number
            && self
                .longitude
                .relative_eq(&other.longitude, epsilon, max_relative)
            && self
                .latitude
                .relative_eq(&other.latitude, epsilon, max_relative)
    }
}
...
pub fn calculate_houses(
    ...
    let (cusps, _ascmc) =
        calculate_house_cusps_swiss(julian_date, latitude, longitude, house_system)?;
...
fn calculate_koch_houses(
    ...
    for i in 0..6 {
        if i != 3 {
            // Skip IC since it's already set
...
fn calculate_alcabitius_houses(
    ...
    for i in 1..6 {
        ...
        let hr2 = if hr2 < 0.0 {
            hr2 + std::f64::consts::PI
        } else {
            hr2
        };
        let hr2 = if hr.sin() < 0.0 {
            hr2 + std::f64::consts::PI
        } else {
            hr2
        };
...
mod tests {
    ...
    fn test_house_systems() {
        ...
        assert_eq!(
            houses.len(),
            12,
            "House system {:?} should return 12 houses",
            house_system
        );
        ...
        for i in 0..houses.len() {
            assert_eq!(
                houses[i].number,
                (i + 1) as u8,
                "House {} should have number {}",
                i + 1,
                i + 1
            );
            let house = &houses[i];
            assert!(
                house.longitude >= 0.0 && house.longitude < 360.0,
                "House {} longitude should be between 0 and 360 degrees",
                i + 1
            );
        }
    }
        ...
        .iter()
        {
            ...
            for (i, house) in houses.iter().enumerate() {
                assert_eq!(
                    house.number,
                    (i + 1) as u8,
                    "House system {:?} should have house {} at index {}",
                    system,
                    i + 1,
                    i
                );
            ...
            for house in houses.iter() {
                assert!(
                    house.longitude >= 0.0 && house.longitude < 360.0,
                    "House system {:?} should have normalized longitudes",
                    system
                );
            }
    ...
    fn test_vedic_houses() {
        ...
        let houses =
            calculate_houses(julian_date, latitude, longitude, HouseSystem::Vedic).unwrap();
        ...
        for (i, house) in houses.iter().enumerate() {
            assert_eq!(
                house.number,
                (i + 1) as u8,
                "Vedic system should have house {} at index {}",
                i + 1,
                i
            );
        }
    ...
    fn test_extreme_latitude_handling() {
        ...
        let _equal_houses =
            calculate_houses(julian_date, latitude, longitude, HouseSystem::Equal).unwrap();
        let _whole_houses =
